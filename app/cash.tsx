...
export default function CashScreen() {
  ...
  const [cashAtHand, setCashAtHand] = useState(values.cash.cashAtHand.toString())
  const [bankAccounts, setBankAccounts] = useState(values.cash.bankAccounts.toString())
  ...
  const totalCash = Number.parseFloat(cashAtHand) + Number.parseFloat(bankAccounts) || 0
  const zakatDue = calculations.cashZakat

  return (
    <SafeAreaView style={{ flex: 1 }}>
      <Text style={styles.title}>Cash & Bank Savings</Text>
      <Text style={styles.subtitle}>Enter your cash at hand and total in all bank accounts.</Text>
      ...
      <ValueDisplay
        items={[
          ...
          { label: "Bank Accounts", value: Number.parseFloat(bankAccounts) || 0 },
        ]}
        total={{
